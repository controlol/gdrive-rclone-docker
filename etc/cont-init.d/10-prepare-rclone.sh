...
for folder in "${folder_arr[@]}"; do
  ...
  if [ ! -f /setupcontainer ]; then
    ...
    echo "0 */6 * * * /usr/bin/rclone rc sync/$upload_command --rc-user=$RC_WEB_USER --rc-pass=$RC_WEB_PASS --json '{\"_filter\":{\"MinAge\":\"6h\"},\"_async\":true,\"srcFs\":\"/local/gdrive/$rclone_folder\",\"dstFs\":\"$rclone_remote\",\"deleteEmptySrcDirs\":true}'" >> /etc/crontabs/root
