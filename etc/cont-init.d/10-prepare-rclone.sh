...
if [ ! -f /setupcontainer ]; then
  ...
  echo "0 */6 * * * /usr/bin/rclone $upload_command /local/gdrive $rclone_remote --config /config/rclone.conf --log-file /var/log/rclone/upload.log --log-level INFO --delete-empty-src-dirs --drive-stop-on-upload-limit --cutoff-mode soft --fast-list --min-age 6h --max-duration 6h" > /etc/crontabs/root
